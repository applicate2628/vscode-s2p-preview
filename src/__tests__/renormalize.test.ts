...
test("keeps unchecked ports at their source reference impedance", () => {
  ...
  const references = effectiveReferenceOhms(doc.referenceOhms, 100, [true, false]);
  const renormalized = renormalizeDocument(doc, 100, [true, false]);
...
test("rejects non-positive target impedance", () => {
  ...
  assert.throws(
    ...
    /Target impedance must be positive/
  );
});
